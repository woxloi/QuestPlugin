...
class QuestStartCommand(private val plugin: JavaPlugin) : CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<String>): Boolean {
        if (sender !is Player) {
            sender.sendMessage(Component.text("$prefix §c§lプレイヤーのみ実行可能です。"))
            return true
        }
        if (args.isEmpty()) {
            sender.sendMessage(Component.text("$prefix §c§lクエストIDを指定してください。"))
            return true
        }
        val id = args[1]
        ...
        if (!success) {
            sender.sendMessage(Component.text("$prefix §c§lクエストをスタートできませんでした"))
            return true
        }
        sender.sendMessage(Component.text("$prefix §a§lクエスト「${quest.name}」を開始しました！"))
        return true
    }
}
